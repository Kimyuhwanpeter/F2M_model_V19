...
def F2M_generator(input_shape=(256, 256, 3), attention_shape=(256, 256, 1)):   # Need to fix parameters!! now!!!!

    h = inputs = tf.keras.Input(input_shape)
    attention_layer = tf.keras.Input(attention_shape)   # 모델 입력에 들어가기전에 reduce_mean 해주어야함!
    ...
    h = tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=3, strides=2, padding="same",
                                        kernel_regularizer=l1_l2, activity_regularizer=l1)(h)
    h = InstanceNormalization()(h * attention_layer + h)  # [256, 256, 64]
    ...
    filters = sharpening_filter(4.0)
    h = tf.pad(h, [[0,0],[2,2],[2,2],[0,0]], "REFLECT")
    h = tf.nn.depthwise_conv2d(h, filter=filters, strides=(1,1,1,1), padding="VALID")

    return tf.keras.Model(inputs=[inputs, attention_layer], outputs=h)
