...
def cal_loss(A2B_model, B2A_model, DA_model, DB_model, DA_age_model, DB_age_model,
    ...
    with tf.GradientTape() as g_tape, tf.GradientTape(persistent=True) as d_tape:
        ...
        fake_B = model_out(A2B_model, [A_batch_images, B_ref], True)
        fake_A_ = model_out(B2A_model, [fake_B, A_ref], True)

        fake_A = model_out(B2A_model, [B_batch_images, A_ref], True)
        fake_B_ = model_out(A2B_model, [fake_A, B_ref], True)
...
def main():
    ...
    A2B_model = F2M_generator(input_shape=(FLAGS.img_size, FLAGS.img_size, 3),
                              attention_shape=(FLAGS.img_size, FLAGS.img_size, 1))
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(train_idx):
                ...
                if count % 100 == 0:
                    fake_B = model_out(A2B_model, [A_batch_images, B_ref], False)
                    fake_A = model_out(B2A_model, [B_batch_images, A_ref], False)
